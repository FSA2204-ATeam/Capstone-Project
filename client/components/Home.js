  ...
  const onMarkerClick = (props, marker, event) => {
    ...
    {{marginleft:'250px'}}>
            ...
            <div classTitle = 'EventList'>
              <ol classTitle='Events'>
                {events.map((event, index)=> 
                  <li key = {index} classTitle='Event' onClick={() => {onLiClick(index)}}>
                    {event.title}
                  </li>
                )}
              </ol>
        ...
        <Map google={props.google} zoom={14}
          initialCenter = {{lat:40.7589, lng:-73.9851}} onClick={onMapClicked}>
          {events.map((marker, i) => 
              <Marker
              ref={(e) => {if (e) markers[i] = 
     e.marker}}
